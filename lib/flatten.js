		...
		item.forEach((element, index) => {
			if (option.keepArray === false && option.currentDepth < option.maximumDepth) {
				let data = flattenObjectPairInternal(
					...
					{
						currentDepth: option.currentDepth + 1,
						currentKey: `${option.currentKey}${(option.currentKey.length === 0) ? "" : option.join}${index}`,
						join: option.join,
				...
				result[`${option.currentKey}${(option.currentKey.length === 0) ? "" : option.join}${index}`] = element;
			};
		});
		...
		Object.keys(item).forEach((keyObject) => {
			if (option.currentDepth < option.maximumDepth) {
				let data = flattenObjectPairInternal(
					...
					{
						currentDepth: option.currentDepth + 1,
						currentKey: `${option.currentKey}${(option.currentKey.length === 0) ? "" : option.join}${keyObject}`,
						join: option.join,
						keepArray: option.keepArray,
						maximumDepth: option.maximumDepth
					}
				...
				result[`${option.currentKey}${(option.currentKey.length === 0) ? "" : option.join}${keyObject}`] = item[keyObject];
			};
		});
	} else {
