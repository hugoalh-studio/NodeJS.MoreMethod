...
function changeCase(item, option = {}) {
	...
	if (runtime.replaceSymbol === true) {
		itemArray = item.replace(/([A-Z])/gu, " $1").replace(/(?:[^\d\s\w]+|_+|[\n\r]+)/giu, " ").split(" ");
	} else {
		itemArray = item.replace(/([A-Z])/gu, " $1").replace(/[-_\n\r\s]+/gu, " ").split(" ");
	};
	...
	if (result.indexOf(runtime.separater) === 0) {
		result = result.replace(runtime.separater, "");
	};
	let positionPhantomTail = result.length - runtime.separater.length;
	if (result.indexOf(runtime.separater, positionPhantomTail) === positionPhantomTail) {
		result = result.slice(0, positionPhantomTail);
	};
	return result.trim();
};
