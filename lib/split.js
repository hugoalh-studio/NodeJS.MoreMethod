...
function split(item, option = {}) {
	...
	if (typeof option.limit !== "undefined") {
		if (option.limit !== Infinity && advancedDetermine.isNumberPositiveInteger(option.limit) !== true) {
			throw new TypeError(`Argument \`option.limit\` must be type of positive integer number!`);
		};
	...
	if (typeof option.separator !== "undefined") {
		if (advancedDetermine.isRegularExpression(option.separator) === true) {
			...
			throw new TypeError(`Argument \`option.separator\` must be type of string (non-nullable) or regular expression!`);
		};
	};
	...
	if (
		"'".search(runtime.separator) !== -1 ||
		"\"".search(runtime.separator) !== -1 ||
		"`".search(runtime.separator) !== -1
	) {
		console.warn(`Argument "option.separator"'s value can cause an unexpected result!`);
	};
	...
	itemArray.forEach((line) => {
		...
		for (let cursorHead = 0; cursorHead < line.length; cursorHead++) {
			if (line.search(runtime.separator) === -1) {
